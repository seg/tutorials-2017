...
The acoustic wave equation with the squared slowness $m$, defined as $m(x,y)=c<sup>{-2}(x,y)$ with $c(x,y)$ being the unknown spatially varying wavespeed, is given by:

$$
\begin{align}
 m \frac{d<sup>2 u(x,y,t)}{dt<sup>2} - \nabla<sup>2 u(x,y,t) + \eta(x,y) \frac{d u(x,y,t)}{dt}=q(x,y,t;x_s, y_s),
\end{align}
$${#WE}
...
$$
 \mathbf{u}[\text{time}+s] = 2\mathbf{u}[\text{time}] - \mathbf{u}[\text{time}-s] + \frac{s<sup>2}{\mathbf{m}} \Big(\Delta \mathbf{u}[\text{time}] + \mathbf{q}[\text{time}]\Big).
$$ {#WEdisa}
...
![T=..33s](Figures/snap1.pdf)
![T=.5s](Figures/snap2.pdf)
![T=..67](Figures/snap3.pdf)
